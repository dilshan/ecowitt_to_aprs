...
/**
 * @brief Constructs and sends an APRS weather report based on the provided WeatherData.
 *
 * This function constructs an APRS weather packet and sends it to the APRS-IS server.
 * It formats various weather parameters such as wind direction, wind speed, temperature,
 * rainfall, solar radiation, humidity, and barometric pressure into the APRS packet.
 * Additionally, it constructs a comment field with information about the weather station
 * model, max daily gust, UV index, battery status, and software name.
 *
...
static void send_aprs_report(struct mg_mgr *mgr, const WeatherData *wd)
{
    ...
    int parsed_items = sscanf(wd->dateutc_str, "%d-%d-%d+%d:%d:%d",
                              &year, &month, &day, &hour, &minute, &second);
    if (parsed_items != 6)
    {
        parsed_items = sscanf(wd->dateutc_str, "%d-%d-%d %d:%d:%d",
                              &year, &month, &day, &hour, &minute, &second);
    }
    if (parsed_items == 6)
    {
        snprintf(dhm_time_str, sizeof(dhm_time_str), "%02d%02d%02d", day, hour, minute);
    }
    else
    {
    ...
    snprintf(aprs_packet_buffer, sizeof(aprs_packet_buffer),
             "%s>%s:@%sz%s/%s_",
             APRS_CALLSIGN_SSID, APRS_DESTINATION, dhm_time_str,
             APRS_LATITUDE, APRS_LONGITUDE);

    char temp_buf[64];

    // Wind direction (cXXX).
    snprintf(temp_buf, sizeof(temp_buf), "c%03d", wd->wind_dir >= 0 && wd->wind_dir <= 360 ? wd->wind_dir : 0);
    strcat(aprs_packet_buffer, temp_buf);

    // Wind speed (sXXX).
    snprintf(temp_buf, sizeof(temp_buf), "s%03d", (int)round(wd->wind_speed_mph < 0 ? 0 : wd->wind_speed_mph));
    strcat(aprs_packet_buffer, temp_buf);

    // Wind gust (gXXX) - this is gust in current observation interval.
    snprintf(temp_buf, sizeof(temp_buf), "g%03d", (int)round(wd->wind_gust_mph < 0 ? 0 : wd->wind_gust_mph));
    strcat(aprs_packet_buffer, temp_buf);

    // Temperature (tXXX or t-XX).
    int temp_f_int = (int)round(wd->temp_f);
    if (temp_f_int < 0 && temp_f_int > -100)
    { // APRS temp is -99 to 999.
        snprintf(temp_buf, sizeof(temp_buf), "t-%02d", (int)round(fabs(wd->temp_f)));
    }
    ...
    { // Out of APRS range, send a boundary or default.
        snprintf(temp_buf, sizeof(temp_buf), "t%03d", temp_f_int < 0 ? -99 : 999);
    }
    strcat(aprs_packet_buffer, temp_buf);

    // Rain last hour (rXXX).
    snprintf(temp_buf, sizeof(temp_buf), "r%03d", (int)round(wd->rain_hourly_in * 100.0));
    strcat(aprs_packet_buffer, temp_buf);

    // Rain last 24 hours (PXXX).
    snprintf(temp_buf, sizeof(temp_buf), "P%03d", (int)round(wd->rain_daily_in * 100.0));
    strcat(aprs_packet_buffer, temp_buf);

    // Solar Radiation (LXXX for W/m^2, or lXXX for W/m^2, if >999 it's kW/m^2 * 10).
    // We'll use LXXX for W/m^2 up to 999.
    if (wd->solar_radiation >= 0 && wd->solar_radiation < 1000)
    {
        snprintf(temp_buf, sizeof(temp_buf), "L%03d", (int)round(wd->solar_radiation));
        strcat(aprs_packet_buffer, temp_buf);
    }
    else if (wd->solar_radiation >= 1000)
    {                                                                                          // Use l for over 999.
        snprintf(temp_buf, sizeof(temp_buf), "l%03d", (int)round(wd->solar_radiation / 10.0)); // Example: 1200W/m^2 -> l120.
        strcat(aprs_packet_buffer, temp_buf);
    }
    // If solar_radiation is 0 or negative, it's omitted from this part.

    // Humidity (hXX).
    snprintf(temp_buf, sizeof(temp_buf), "h%02d", wd->humidity == 100 ? 0 : (wd->humidity < 0 ? 0 : wd->humidity % 100));
    strcat(aprs_packet_buffer, temp_buf);

    // Barometric pressure (bXXXXX).
    snprintf(temp_buf, sizeof(temp_buf), "b%05d", (int)round(wd->baro_rel_in * 33.8639 * 10.0));
    strcat(aprs_packet_buffer, temp_buf);
    ...
    {
        strncat(comment_buffer, wd->model, sizeof(comment_buffer) - strlen(comment_buffer) - 1);
    ...
    {
        snprintf(temp_buf, sizeof(temp_buf), "MaxGust:%.1fmph ", wd->max_daily_gust_mph);
        strncat(comment_buffer, temp_buf, sizeof(comment_buffer) - strlen(comment_buffer) - 1);
    }
    ...
    strncat(comment_buffer, APRS_SOFTWARE_NAME, sizeof(comment_buffer) - strlen(comment_buffer) - 1);

    // Remove trailing space if any.
    if (strlen(comment_buffer) > 0 && comment_buffer[strlen(comment_buffer) - 1] == ' ')
    {
    ...
    {
        strncat(aprs_packet_buffer, comment_buffer, sizeof(aprs_packet_buffer) - strlen(aprs_packet_buffer) - 1);
    }

    // 4. Construct the full POST body for APRS-IS.
    snprintf(post_body, sizeof(post_body),
             "user %s pass %s vers %s %s\n%s\n",
             APRS_CALLSIGN_SSID, APRS_PASSCODE,
             APRS_SOFTWARE_NAME, APRS_SOFTWARE_VERSION, aprs_packet_buffer);

    printf("APRS: Preparing to send:\n%s", post_body);

    // 5. Send to APRS-IS server.
    char aprs_server_url[120];
    snprintf(aprs_server_url, sizeof(aprs_server_url), "http://%s:%d/", APRS_SERVER_HOST, APRS_SERVER_PORT);

    struct mg_connection *aprs_conn = mg_http_connect(mgr, aprs_server_url, NULL, NULL);
    if (aprs_conn == NULL)
    {
 ...
 *
 * This function processes incoming HTTP messages, extracts weather data from POST requests,
 * and sends the data to the APRS-IS server if valid. It also handles GET requests and
...
static void fn(struct mg_connection *c, int ev, void *ev_data)
{
    if (ev == MG_EV_HTTP_MSG)
    {
        ...
        current_wx_data.solar_radiation = -1; // Initialize to indicate not present.
        ...
        if (mg_strcmp(hm->method, mg_str("POST")) == 0)
        {
            ...
            if (content_type_hdr != NULL &&
                mg_strcmp(*content_type_hdr, mg_str("application/x-www-form-urlencoded")) == 0)
            {

                printf("Received POST request to / with Content-Type: application/x-www-form-urlencoded\n");
                ...
                while (current_pos < end_of_body)
                {
                    ...
                    int key_len_decoded = mg_url_decode(key_encoded.buf, key_encoded.len, key_buf, sizeof(key_buf) - 1, 1);
                    if (key_len_decoded >= 0)
                        key_buf[key_len_decoded] = '\0';
                    else
                        strcpy(key_buf, "(KEY_ERR)");

                    int value_len_decoded = mg_url_decode(value_encoded.buf, value_encoded.len, value_buf, sizeof(value_buf) - 1, 1);
...
#pragma GCC diagnostic ignored "-Wstringop-truncation"
                    ...
                    if (strcmp(key_buf, "dateutc") == 0)
                        strncpy(current_wx_data.dateutc_str, value_buf, sizeof(current_wx_data.dateutc_str) - 1);
                        ...
                        strncpy(current_wx_data.model, value_buf, sizeof(current_wx_data.model) - 1);
...
#pragma GCC diagnostic pop
                ...
                if (strlen(current_wx_data.dateutc_str) > 0 && current_wx_data.baro_rel_in > 0)
                {
...
int main(void)
{
    ...
    printf("APRS Callsign: %s, Lat: %s, Lon: %s\n", APRS_CALLSIGN_SSID, APRS_LATITUDE, APRS_LONGITUDE);
